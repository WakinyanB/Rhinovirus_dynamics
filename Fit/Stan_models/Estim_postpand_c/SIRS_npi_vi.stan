data{
  ...
  int tmax_npi;         // Last week index with Google mobility data (October 15, 2022)
...
parameters{
  ...
  real<lower=0> eta; // transmission multiplier post-pandemic
...
transformed parameters{
  ...
  for(i in 1:n_week){
    ...
    real h = (1+kappa*c[i])*(1+phi*obs_IAV[i])*beta[week[i]]*I[i]/pop;
    if(i>tmax_npi){
      h = eta*h;
    }
