data{
  ...
  int<lower=1> npi_period[2]; // First and last week number for NPIs
...
parameters{
  ...
  vector<lower=0, upper=1>[npi_period[2]-npi_period[1]+1] npi;
...
transformed parameters{
  ...
  for(i in 1:n_week){
    ...
    real h = (1+phi*obs_IAV[i])*beta[week[i]]*I[i]/pop;
    
    if(i >= npi_period[1] && i <= npi_period[2]){ // Pandemic NPIs
      h = h*npi[i-npi_period[1]+1];
    }
