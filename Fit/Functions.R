...
Plot_beta <- function(beta, ylim, ncol, dt=1, mu=1/80/52, gamma=2){
  return(
    beta %>%
      ...
      labs(y=expression(paste(beta, ", transmission rate (",week^{-1},")"))) +
      ...
      scale_y_continuous(expand=c(0,0), limits=ylim,
                         sec.axis=sec_axis(
                           ...
                           name=expression(paste(R[0], ", basic reproduction number")))) +
      theme_bw() +
...
plot_SI_fit2 <- function(SI, fit, Re, location,
  ...
  label_col <- c('estimated', '0 (fixed)')
  SI$vi <- factor(SI$vi, levels=c('1','0'), labels=label_col)
  fit$vi <- factor(fit$vi, levels=c('1','0'), labels=label_col)
  
  my_theme <- theme_test() +
              theme(plot.margin=unit(c(0.1,0.2,0.15,0.1),"cm"), axis.title.x=element_blank(),
  ...
  return(
    plot_grid(
      ggplot(subset(SI, var=='S'), aes(x=date)) +
        ...
        my_theme +
        theme(plot.title=element_text(size=15), legend.position=c(0.05,0.9),
      ...
      ggplot(Re, aes(x=date)) +
        ...
        my_theme +
        theme(legend.position=c(0.03,0.95), legend.direction='horizontal',
