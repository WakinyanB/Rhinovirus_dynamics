...
fit <- readRDS("Fit/Output/fit_RV_SIRS_npi_vi_all_locations_hierarchical.RDS")
...
phi_estim <- (1:n_loc) %>%
  lapply(function(i){return(data.frame(location=regions[i], phi=p$phi[,i]))}) %>%
  bind_rows

phi_quantiles <- compute_median_95CI(p$phi) %>%
  cbind(location=regions) %>%
  mutate(location=factor(location, levels=regions))

mu_phi_estim <- rbind(data.frame('country'='US', 'phi'=p$mu_phi[,1]),
                      data.frame('country'='Canada', 'phi'=p$mu_phi[,2])) %>%
  mutate(country=factor(country, levels=c('Canada', 'US')))

mu_phi_quantiles <- compute_median_95CI(p$mu_phi) %>%
  cbind(country=c('US', 'Canada')) %>%
  mutate(country=factor(country, levels=c('Canada', 'US')))

(Fig_mu_phi <- ggplot(mu_phi_estim, aes(x=phi, y=country)) +
    labs(x="\nMax. change in transmission due to viral interaction (mean)\n") +
    geom_vline(xintercept=0, lwd=0.2, lty='dashed') +
    geom_violin(fill="#800080", col=NA, alpha=0.2) +
    geom_segment(data=mu_phi_quantiles, aes(x=CI_lower, xend=CI_upper), lwd=0.2) +
    geom_point(data=mu_phi_quantiles, aes(x=median), pch=21, fill="#800080", cex=2) +
...
plot_grid(
  Fig_fit,
  plot_grid(
    plot_grid(Fig_phi, Fig_Omega, ncol=1, rel_heights=c(0.6,0.4), labels=LETTERS[2:3]),
    Fig_R0,
    ncol=2, rel_widths=c(0.45,0.55), labels=c(NA, 'D')),
  ncol=1, rel_heights=c(0.62,0.38), labels='A')

# ggsave('Results_fit_combined_hierarchical.pdf', width=9, height=11)
