...
plot(b0_rv*(1+a1_rv*cos(4*pi*((1:52)/52-d1_rv))+a2_rv*cos(2*pi*((1:52)/52-(d1_rv+d2_rv))))/gamma_rv,
     ylim=c(1.2,2.5), type='l', col='blue')
...
write.csv2(SIR, "SIR_3.csv", row.names=FALSE)
saveRDS(parms, "parms_simul_3.rds")
...
save(obs_rv, obs_iav, file="cases_3.RData")
...
save(obs_rv, obs_iav, file="cases_3_SDhigh.RData")
...
write.csv2(SIR_kick, "SIR_kick_3.csv", row.names=FALSE)
...
save(obs_kick_rv, obs_kick_iav, file="cases_kick_3.RData")
...
write.csv2(SIR_shift, "SIR_shift_3.csv", row.names=FALSE)
...
save(obs_shift_rv, obs_shift_iav, file="cases_shift_3.RData")