...
parms0 <- readRDS("parms_simul_1.rds")
SIR0 <- read.csv2("SIR_1.csv") %>% subset(pathogen=="IAV")
...
beta_rv <- beta0/(1+phi_iav*SIR0$I[2:53]/pop)

plot(beta_rv/parms0[["gamma_rv"]], ylim=c(1.2,3.2), type='l', col='blue')
...
parms <- list("beta_rv"=beta_rv,
              "gamma_rv"=parms0[["gamma_rv"]], "omega_rv"=parms0[["omega_rv"]],
              "phi_rv"=parms0[["phi_rv"]], "rho_rv"=parms0[["rho_rv"]],
              "k_rv"=parms0[["k_rv"]],
              "b0_iav"=b0_iav, "a_iav"=a_iav, "d_iav"=d_iav, "gamma_iav"=gamma_iav,
              "omega_iav"=omega_iav, "phi_iav"=phi_iav, "k_iav"=k_iav,
              "rho_iav"=rho_iav,
              "mu"=parms0[["mu"]], "pop"=pop)
...
# write.csv2(SIR, "SIR_2_no_vi.csv", row.names=FALSE)
# saveRDS(parms, "parms_simul_2_no_vi.rds")
...
save(obs_rv, obs_iav, file="cases_2_no_vi.RData")
...
# write.csv2(SIR_kick, "SIR_kick_2_no_vi.csv", row.names=FALSE)
...
save(obs_kick_rv, obs_kick_iav, file="cases_kick_2_no_vi.RData")
...
# write.csv2(SIR_shift, "SIR_shift_2_no_vi.csv", row.names=FALSE)
...
save(obs_shift_rv, obs_shift_iav, file="cases_shift_2_no_vi.RData")
