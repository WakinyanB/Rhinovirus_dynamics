...
plot(b0_rv*(1+a1_rv*cos(4*pi*((1:52)/52-d1_rv))+a2_rv*cos(2*pi*((1:52)/52-(d1_rv+d2_rv))))/gamma_rv,
     type='l', col='blue', ylim=c(1.3,2.2), ylab='Basic reproduction number')
lines(b0_iav*(1+a_iav*cos(2*pi*(1:52)/52-d_iav))/gamma_iav, col='red')
...
write.csv2(SIR, "SIR_1.csv", row.names=FALSE)
saveRDS(parms, "parms_simul_1.rds")
...
write.csv2(SIR_kick, "SIR_kick_1.csv", row.names=FALSE)
...
save(obs_kick_rv, obs_kick_iav, file="cases_kick_1.RData")
...
write.csv2(SIR_shift, "SIR_shift_1.csv", row.names=FALSE)
...
save(obs_shift_rv, obs_shift_iav, file="cases_shift_1.RData")